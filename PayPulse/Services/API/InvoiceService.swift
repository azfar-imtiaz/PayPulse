...
class InvoiceService {
    ...
    /// Fetches and decodes subscription invoice details
    func getSubscriptionDetail(invoiceID: String) async throws -> SubscriptionDetail {
        // Create a custom response structure for this specific detail type
        struct SubscriptionDetailWrapper: Codable {
            let invoiceDetails: SubscriptionDetail
        }

        let response: APISuccessResponse<SubscriptionDetailWrapper> = try await apiClient.request(
            path: "invoices/\(InvoiceType.retail.apiPath)",
            method: .get,
            parameters: [
                "subtype": RetailInvoiceSubType.subscriptions.apiPath,
                "invoice-id": invoiceID
            ],
            encoding: URLEncoding.queryString
        )
        ...
        guard let responseData = response.data else {
            throw APIError.decodingError(NSError(
                domain: "InvoiceService",
                code: -1,
                userInfo: [NSLocalizedDescriptionKey: "No subscription detail data returned"]
            ))
        }
    ...
    /// Fetches and decodes clothing invoice details
    func getClothingDetail(invoiceID: String) async throws -> ClothingDetail {
        // Create a custom response structure for this specific detail type
        struct ClothingDetailWrapper: Codable {
            let invoiceDetails: ClothingDetail
        }

        let response: APISuccessResponse<ClothingDetailWrapper> = try await apiClient.request(
            path: "invoices/\(InvoiceType.retail.apiPath)",
            method: .get,
            parameters: [
                "subtype": RetailInvoiceSubType.clothing.apiPath,
                "invoice-id": invoiceID
            ],
            encoding: URLEncoding.queryString
        )
        ...
        guard let responseData = response.data else {
            throw APIError.decodingError(NSError(
                domain: "InvoiceService",
                code: -1,
                userInfo: [NSLocalizedDescriptionKey: "No clothing detail data returned"]
            ))
        }
    ...
    /// Fetches and decodes travel invoice details
    func getTravelDetail(invoiceID: String) async throws -> TravelDetail {
        // Create a custom response structure for this specific detail type
        struct TravelDetailWrapper: Codable {
            let invoiceDetails: TravelDetail
        }

        let response: APISuccessResponse<TravelDetailWrapper> = try await apiClient.request(
            path: "invoices/\(InvoiceType.retail.apiPath)",
            method: .get,
            parameters: [
                "subtype": RetailInvoiceSubType.travel.apiPath,
                "invoice-id": invoiceID
            ],
            encoding: URLEncoding.queryString
        )
        ...
        guard let responseData = response.data else {
            throw APIError.decodingError(NSError(
                domain: "InvoiceService",
                code: -1,
                userInfo: [NSLocalizedDescriptionKey: "No travel detail data returned"]
            ))
        }
    ...
    /// Fetches and decodes technology invoice details
    func getTechnologyDetail(invoiceID: String) async throws -> TechnologyDetail {
        // Create a custom response structure for this specific detail type
        struct TechnologyDetailWrapper: Codable {
            let invoiceDetails: TechnologyDetail
        }

        let response: APISuccessResponse<TechnologyDetailWrapper> = try await apiClient.request(
            path: "invoices/\(InvoiceType.retail.apiPath)",
            method: .get,
            parameters: [
                "subtype": RetailInvoiceSubType.technology.apiPath,
                "invoice-id": invoiceID
            ],
            encoding: URLEncoding.queryString
        )
        ...
        guard let responseData = response.data else {
            throw APIError.decodingError(NSError(
                domain: "InvoiceService",
                code: -1,
                userInfo: [NSLocalizedDescriptionKey: "No technology detail data returned"]
            ))
        }
